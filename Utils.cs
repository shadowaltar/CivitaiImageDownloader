...
public static class Utils
{
    ...
    public static int GetInt(this JsonNode? node, string nodeName)
    {
        return int.TryParse(node?[nodeName]?.AsValue().ToString(), out var result) ? result : -1;
    }

    public static string GetStr(this JsonNode? node, string nodeName)
    {
        return node?[nodeName]?.AsValue().ToString() ?? "";
    }
}
